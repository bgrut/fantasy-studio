...
# ---------------------------------------------------------------------------
# Scale reference: ocean creatures are typically large
# ---------------------------------------------------------------------------
_CREATURE_TARGET_SIZE: dict[str, float] = {
    "whale":   6.0,
    "shark":   3.0,
    "fish":    0.5,
    "creature": 1.5,
...
def build_ocean_scene(bpy, manifest: dict, scene) -> None:
    ...
    for idx, asset in enumerate(creatures[:5]):
        ...
        # Determine target size from registry species field or tag fallback
        ...
        if not species:
            for sp in ("whale", "shark", "fish"):
                if sp in tag_set:
                    species = sp
                    break
        target_size = _CREATURE_TARGET_SIZE.get(species, _CREATURE_TARGET_DEFAULT)
