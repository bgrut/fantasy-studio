...
def build_street_scene(bpy, manifest: dict, scene) -> None:
    ...
    for i, asset in enumerate(characters[:char_count]):
        ...
        ok, meshes = import_and_place_asset_group(
            ...
            ground_z=0.0,
            axis_mode="height",   # height-driven for all character types
        )
        if ok:
    ...
    # If more slots than resolved assets, fill remaining with fallbacks
    for i in range(len(characters), char_count):
        fallback_needed.append(i)

    # Fallback stand-in characters for any unresolved slots
    if fallback_needed:
