...
# Used to build a deterministic, extensible bonus/penalty matrix.
# ---------------------------------------------------------------------------
_SPECIES_AFFINITY: list[tuple[str, str, int]] = [
    # positive matches — prompt species → correct asset tag
    ("cat",       "cat",       60),
    ("cat",       "feline",    50),
    ("cats",      "cat",       60),
    ("cats",      "feline",    50),
...
# Cross-species penalties — prompt species → wrong asset tag, penalty amount.
# Keeps irrelevant animals out of scenes.
...
def _tag_overlap_score(asset_tags: list[str], wanted_tags: list[str]) -> int:
    ...
    return len(a.intersection(b)) * 10  # weight tag overlap clearly above noise


def _name_match_score(asset: dict, wanted_tags: list[str]) -> int:
    """
    Score based on whether any wanted tag appears in the asset's name or id.
    Catches assets like 'cat_dancer_v2' whose tags might be incomplete.
    """
...
def _extract_prompt_species(manifest: dict) -> set[str]:
    """
    Pull species keywords directly from the raw prompt and debug_notes —
    not from the full scene_plan JSON blob (which is too broad).
    """
    raw = " ".join([
    ...
    # Also read subject_species hint written by prompt_scene_planner
...
def _species_score(asset: dict, prompt_species: set[str]) -> int:
    """
    Apply the affinity and penalty tables based on detected prompt species.
    Returns net bonus/penalty for this asset.
    """
...
def _select_best_models(
    ...
    for item in candidates:
        ...
        # Secondary sort key is asset name for determinism across equal scores
        secondary = str(item.get("name", item.get("id", "")))
        scored.append((score, secondary, item))

    scored.sort(key=lambda x: (-x[0], x[1]))

    # Deduplicate by asset id/name so count > distinct candidates doesn't repeat
