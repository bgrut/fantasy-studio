...
# ---------------------------------------------------------------------------
# Forward-axis auto-detection
# Vehicles may have their nose pointing along +Y or +X depending on how the
# asset was modelled.  The registry can express this via "forward_axis": "Y"
# or "forward_axis": "X".  When the field is absent we infer from bounds.
# ---------------------------------------------------------------------------

def detect_forward_axis(meshes, depsgraph=None) -> str:
    """
    Guess the forward (nose) axis of a vehicle mesh group by comparing
    the X and Y extents.  Vehicles are typically longer along one axis.

    Returns "Y" (default, most .blend vehicles exported nose-forward) or "X".
    If bounds are unavailable returns "Y" as safe default.
    """
    mins, maxs = bounds_world(meshes, depsgraph)
    if mins is None:
        return "Y"
    size = maxs - mins
    # Whichever horizontal axis is longer is the forward axis.
    # Ties and near-square cross-sections default to "Y".
    if size.x > size.y * 1.15:
        return "X"
    return "Y"


def axis_mode_for_asset(asset: dict) -> str:
    """
    Resolve the correct axis_mode string for normalize_root_group.
    Checks asset["forward_axis"] first (registry override), then falls back
    to the _AXIS_MODE table keyed on asset["type"].
    """
    fwd = str(asset.get("forward_axis") or "").upper()
    if fwd == "X":
        return "width"   # nose along X → normalise on x-dimension
    if fwd == "Y":
        return "length"  # nose along Y → normalise on y-dimension (default)

    asset_type = str(asset.get("type") or "").lower()
    return _AXIS_MODE.get(asset_type, "max")
...
def frame_camera_to_meshes(
    ...
    - target (look-at Empty) is updated to the computed aim point so any
      ...
      A view_layer.update() is called after repositioning the target so
      Blender's constraint graph re-evaluates before the camera is placed.
    ...
    # Force the constraint graph to re-evaluate after repositioning the target.
    # Without this, a TRACK_TO constraint on the camera still points at the
    # old target location when we set cam.location below.
    try:
        import bpy as _bpy
...
# Registry forward_axis values → Blender TRACK_TO track_axis setting.
# A car stored with nose pointing +Y → track_axis = 'TRACK_NEGATIVE_Y'
# when camera faces the car (camera looks toward +Y direction).
_FORWARD_AXIS_TO_TRACK: dict[str, str] = {
    "+y":  "TRACK_NEGATIVE_Y",
    "-y":  "TRACK_Y",
    "+x":  "TRACK_NEGATIVE_X",
    "-x":  "TRACK_X",
    "y":   "TRACK_NEGATIVE_Y",   # shorthand aliases
    "-y":  "TRACK_Y",
    "x":   "TRACK_NEGATIVE_X",
    "-x":  "TRACK_X",
}
...
def detect_forward_axis(meshes, depsgraph=None) -> str:
    """
    Auto-detect which horizontal axis is the "forward" (nose) direction
    of an asset by comparing X and Y extent.

    Heuristic: a vehicle is almost always longer in one horizontal axis.
    We return the axis with the longer span as the forward axis.

    Returns one of: '+y', '-y', '+x', '-x'
    Default: '+y'  (Blender convention for most imported vehicles)
    """
    mins, maxs = bounds_world(meshes, depsgraph)
    if mins is None:
        return "+y"
    ...
    size = maxs - mins
    # If Y-span is dominant → forward is Y (Blender default)
    if size.y >= size.x:
        return "+y"
    else:
        return "+x"
...
def axis_mode_for_asset(asset: dict, fallback: str = "max") -> str:
    """
    Return the correct normalization axis_mode for this asset.
    Priority:
      1. asset["forward_axis"] field (set in registry for vehicles)
      2. asset["type"] → _AXIS_MODE table
      3. fallback
    """
    fa = str(asset.get("forward_axis") or "").lower()
    if fa in ("+y", "-y", "y"):
        return "length"   # Y-dominant → normalise on length
    if fa in ("+x", "-x", "x"):
        return "width"    # X-dominant → normalise on width
...
def filter_useful_meshes(meshes, min_volume: float = 0.001) -> list:
    """
    Remove meshes that are clearly debris, helper planes, or near-zero-volume
    objects that would corrupt camera framing.

    A mesh is considered "useful" if its world-space bounding box volume
    exceeds min_volume (default 0.001 Blender-unit³).  This filters out
    single-vertex helpers, control empties exported as meshes, and UV-unwrap
    seam planes that some .blend files include.

    Parameters
    ----------
    meshes      List of Blender MESH objects.
    min_volume  Minimum axis-aligned bounding-box volume to keep.
                0.001 ≈ 0.1m × 0.1m × 0.1m — anything smaller is junk.
    """
    useful = []
    for obj in (meshes or []):
        ...
        try:
            ...
            useful.append(obj)   # include on error rather than drop
    return useful or meshes  # never return empty if input was non-empty
