...
const feel = await (async () => {
  ...
  const gaitW = await p.evaluate(() => { const f = window.__game.facts(); return { gait: f.gait, lean: f.lean, arms: f.arms, ride: f.ride, shoulders: f.shoulders }; });
...
console.log('the gait  : walking', JSON.stringify(feel.gaitW.gait), '| idle again', JSON.stringify(feel.idleW), '| turning roll', feel.turning.roll, '| head bone', feel.gaitW.lean.head_bone, '| arms from down', JSON.stringify(feel.gaitW.arms), '| hip ride', feel.gaitW.ride, '| shoulders', JSON.stringify(feel.gaitW.shoulders));
...
const ok = r.landmark && r.landmark.w > 5 && r.landmark.h > 5 && errs.length === 0
  ...
  && feel.gaitW.gait.walk > 0.5 && feel.gaitW.gait.idle < 0.5 && feel.gaitW.gait.rate >= 0.5 && feel.gaitW.gait.rate <= 5.5 && feel.idleW.idle > 0.9 && Math.abs(feel.turning.roll) > 0.01 && !!feel.gaitW.lean.head_bone && feel.gaitW.arms && feel.gaitW.arms.L !== null && feel.gaitW.arms.L < 32 && feel.gaitW.arms.R < 32 && feel.gaitW.ride >= 0.015 && feel.gaitW.ride <= 0.09 && feel.gaitW.shoulders && feel.gaitW.shoulders.L !== null && feel.gaitW.shoulders.L < 6 && feel.gaitW.shoulders.R < 6   // no shrug   // the arms hang and swing, no chicken wings; the pelvis rides three to six centimetres   // a detective with a weapon holds the pistol; a build with no hostiles holds nothing && cast.ghosts > 0 && cast.ghosts <= 3 && cast.animals === 0   // a haunting has ghosts, not wolves, and not a crowd
