...
window.__shoot = () => {
  ...
  for (let k = 0; k < 4; k++) {
    ...
    cam.position.set(0, h * 0.55, h * 2.6); cam.lookAt(0, h * 0.5, 0);
...
const out = path.join(HERE, 'renders', 'rig_' + path.basename(rel, '.glb') + '_' + clip + '.png'); fs.writeFileSync(out, Buffer.from(png.split(',')[1], 'base64'));
