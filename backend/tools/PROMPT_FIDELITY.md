...
Scored on 2026-09-21 by `backend/tools/prompt_check.py`: each sentence carries what a careful reader would expect from it, and the build is scored on how many of those it delivered, read from the resolved spec the studio wrote. The studio's own off-brief notes ride along.

**37 of 39 expectations met (95%).** Pass.

| the sentence | title | genre | score | missed | stand-ins | off-brief |
|---|---|---|---:|---|---|---|
| a haunted manor on a windswept moor, find the three relics before the bell tolls | The Moor's Haunting | adventure | 7/7 |  | hero cast: 'man' is played by the detective, dressed for this world (name a character in the prompt to choose); 'woman' isn't in your library, so the guide is played by 'man': the level keeps its guides |  |
| a tokyo drift racing game through neon streets at night | Tokyo Drift Nights | adventure | 5/5 |  | 'woman' isn't in your library, so the guide is played by 'man': the level keeps its guides |  |
| a rusted mining outpost on a dead red moon | Red Moon Outpost | factory | 2/2 |  | hero cast: 'miner' is played by the explorer, dressed for this world (name a character in the prompt to choose) |  |
...
| a scientist collecting samples in a toxic swamp full of crocodiles | Swamp Samples | adventure | 3/4 | hostile crocodile: snake | the crocodile is played by the snake; 'woman' isn't in your library, so the guide is played by 'man': the level keeps its guides |  |
