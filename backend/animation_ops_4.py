...
def _fallback_turntable(objs, frame_end: int, offset: int = 0) -> None:
    ...
    Rotates relative to the current resting orientation so placement
    rotations applied by normalize_root_group are preserved.
    """
    for obj in objs:
        try:
            base_rz = obj.rotation_euler.z   # preserve any placement rotation
...
def _fallback_glide(objs, frame_end: int, offset: int = 0,
    ...
    """Forward glide on Y for environment dressing (non-vehicle generic)."""
    for obj in objs:
        try:
            base_y = obj.location.y
...
def _fallback_vehicle_glide(objs, frame_end: int, offset: int = 0,
                            travel: float = 6.0) -> None:
    """
    Cinematic vehicle glide: forward Y travel + subtle body-roll tilt.
    The car starts behind camera-left, crosses to camera-right, with a
    slight Z-rotation lean into the direction of travel for dynamism.
    Location is restored after keyframing so the resting pose is preserved.
    """
    import math
    lean = math.radians(1.8)   # subtle body roll — enough to read, not cartoonish
    for obj in objs:
        try:
            base_y  = obj.location.y
            base_rz = obj.rotation_euler.z
            ...
            # Start position: pulled back half travel
            obj.location.y      = base_y - travel * 0.5
            obj.rotation_euler.z = base_rz - lean   # lean into entry
            obj.keyframe_insert(data_path="location",       index=1, frame=f1)
            obj.keyframe_insert(data_path="rotation_euler", index=2, frame=f1)

            # End position: pushed forward half travel
            obj.location.y      = base_y + travel * 0.5
            obj.rotation_euler.z = base_rz + lean   # lean into exit
            obj.keyframe_insert(data_path="location",       index=1, frame=f2)
            obj.keyframe_insert(data_path="rotation_euler", index=2, frame=f2)

            # Linear interpolation → constant-speed glide, no ease-in/out jitter
            if obj.animation_data and obj.animation_data.action:
                for fc in obj.animation_data.action.fcurves:
...
_FALLBACK_PROFILES: dict[str, tuple] = {
    ...
    "vehicle_glide":  (_fallback_vehicle_glide,   {"travel": 6.0}),   # car hero motion
            ...
            for arm in armatures:
                ...
                if existing_actions:
                    ...
                    if pushed:
                        ...
                        nla_pushed = True
                        break   # one action per instance is enough

        if nla_pushed:
            pass   # NLA handles it — skip procedural fallback
        elif armatures:
